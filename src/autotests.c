...
void testReadAMS(){
	...
	for(k=0;k<16;k++){
		memcpy(mySongRef.tickTab[k].note,array[k],sizeof(int)*4);
...
void testReadAMP(){
    ...
    int coeff[AMP_NB_TESTS] = {1,1,1,3,1,1,3,3};
   ...
   // if (pf != NULL){
    ...
    //}
		...
		while(!feof(pf)){ // read amp file line by line
			...
			if (!strcmp("let_s_eat.ams", songfilename) && nbtest == 6){
				mark[nbtest]=1;
				#ifdef DEBUG
				printf("test %d AMP ok\n", nbtest);
				#endif
			}
			if (!strcmp("my_song.ams", songfilename) && nbtest == 7){
				mark[nbtest]=1;
				#ifdef DEBUG
				printf("test %d AMP ok\n", nbtest);
				#endif
...
void testCreateAMS(){
	...
	if(pf1!=NULL){
		printf("DESTRUCTION DU FICHIER TEST.ams avant les autotests ! \n");
		fclose(pf1);
		remove(amsFileName);
	}
